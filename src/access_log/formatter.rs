...
fn format_text(entry: &AccessLogEntry) -> Vec<u8> {
  ...
  write!(line, "{}:{}", entry.client_ip, entry.client_port).unwrap();

  // Field 2: user or "-"
  write!(line, " {}", entry.user.as_deref().unwrap_or("-")).unwrap();

  // Field 3: timestamp in nginx format [dd/Mon/yyyy:HH:mm:ss +zzzz]
  let time_str = entry
    .time
    .format(&TEXT_TIME_FORMAT)
    .unwrap_or_else(|_| "???".to_string());
  write!(line, " [{time_str}]").unwrap();

  // Field 4: "method target"
  write!(line, " \"{} {}\"", entry.method, entry.target).unwrap();
...
fn format_json(entry: &AccessLogEntry) -> Vec<u8> {
  ...
  let time_str =
    entry.time.format(&format).unwrap_or_else(|_| "???".to_string());
  map.insert("time".to_string(), serde_json::Value::String(time_str));
  ...
  map.insert("status".to_string(), serde_json::json!(entry.status));
  map.insert(
...
mod tests {
  ...
  fn test_text_format_contains_client_addr() {
    ...
    let output =
      String::from_utf8(format_entry(&entry, LogFormat::Text)).unwrap();
    assert!(
  ...
  fn test_text_format_no_user_shows_dash() {
    ...
    assert_eq!(parts[1], "-", "No user should show '-', got: {output}");
  }
  ...
  fn test_json_format_service_metrics_sorted_deterministic() {
    ...
    let alpha_pos =
      output.find("service.alpha").expect("should have alpha");
    let middle_pos =
      output.find("service.middle").expect("should have middle");
    let zebra_pos =
      output.find("service.zebra").expect("should have zebra");
