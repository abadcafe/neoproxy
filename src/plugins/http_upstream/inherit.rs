/// Resolve a field through three-level inheritance: address > upstream > plugin.
/// Returns the first `Some` value found.
...
/// Like `resolve_field`, but falls back to a default if all levels are `None`.
...
mod tests {
  ...
  fn test_resolve_field_address_wins() {
    assert_eq!(
      resolve_field(Some(&1), Some(&2), Some(&3)),
      Some(1)
    );
  }
  ...
  fn test_resolve_field_upstream_fallback() {
    assert_eq!(
      resolve_field(None, Some(&2), Some(&3)),
      Some(2)
    );
  }
  ...
  fn test_resolve_field_plugin_fallback() {
    assert_eq!(
      resolve_field(None, None, Some(&3)),
      Some(3)
    );
  }
  ...
  fn test_resolve_field_with_default_uses_default() {
    assert_eq!(
      resolve_field_with_default(None, None, None, 99),
      99
    );
  }
}
