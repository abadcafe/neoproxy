...
mod tests {
  use super::*;
  use crate::plugins::js_sandbox::request::IncomingRequest;
  ...
  fn test_sandbox_simple_response() {
    ...
    let sandbox =
      Sandbox::new(config).expect("sandbox creation failed");
    ...
    let resp =
      sandbox.execute(request).expect("sandbox execute failed");
