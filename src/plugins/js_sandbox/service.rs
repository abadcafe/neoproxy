...
impl SandboxService {
  ...
  #[cfg(test)]
  fn new_without_pool(config: Arc<PluginConfig>) -> Self {
...
mod tests {
  ...
  /// Lightweight service without pool — only works for error-path tests
  /// that return before reaching sandbox execution.
  fn make_light_service(source_dir: &Path) -> SandboxService {
    SandboxService::new_without_pool(Arc::new(make_plugin_config(
      source_dir,
    )))
  }
  ...
  fn test_poll_ready_returns_ready() {
    ...
    let mut svc = make_light_service(tmp.path());
...
"#,
    ...
    assert_eq!(resp.status(), 200);
    assert_eq!(
      resp.headers().get("x-a").unwrap().to_str().unwrap(),
      "1"
    );
    assert_eq!(
      resp.headers().get("x-b").unwrap().to_str().unwrap(),
      "2"
    );
    assert_eq!(
      resp.headers().get("x-c").unwrap().to_str().unwrap(),
      "3"
    );
  }
...
"#
    ...
    if status != 200 {
      eprintln!(
        "outbound fetch test: status={}, body={}",
        status, body
      );
    }
