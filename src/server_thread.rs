...
/// A group of servers sharing the same address and listener name.
...
async fn server_thread_main(shutdown: Arc<sync::Notify>) -> Result<()> {
  ...
  // Collect service names referenced by at least one server
  let referenced_services: std::collections::HashSet<&str> =
    Config::global()
      .servers
      .iter()
  ...
  // Build only services referenced by servers
  for sc in Config::global().services.iter() {
    if !referenced_services.contains(sc.name.as_str()) {
  ...
  // Group servers by (address, listener_name) for shared-address listeners
  let listener_groups = group_servers_by_address_listener_name(
  ...
  for group in listener_groups {
    let builder = ListenerBuilderSet::global()
      .listener_builder(&group.listener_name)
...
mod tests {
  ...
  fn test_plugin_set_entry_occupied() {
    ...
    let _ = plugin_set.get_or_create_plugin("echo");
    assert_eq!(plugin_set.len(), 1); // Still only 1 plugin
  }
  ...
  #[test]
  fn test_group_servers_by_address_listener_name_single_server() {
    ...
    let mut services = HashMap::new();
    services.insert("echo", placeholder_service());
    ...
    let groups = group_servers_by_address_listener_name(
      &servers,
      &services,
      &access_log_writers,
    );
    ...
    assert_eq!(groups[0].listener_name, "http");
  }

  #[test]
  fn test_group_servers_by_address_listener_name_shared_address() {
    // Two servers sharing the same address+listener_name
    let servers = vec![
      crate::config::Server {
        name: "default".to_string(),
        hostnames: vec![],
    ...
    let mut services = HashMap::new();
    services.insert("echo", placeholder_service());
    services.insert("api_service", placeholder_service());
    ...
    // Both servers should be in the same group (same address+listener_name)
    assert_eq!(groups.len(), 1);
  ...
  fn test_group_servers_by_address_listener_name_different_listener_names()
   {
    ...
    // Should have two different groups (different listener names)
