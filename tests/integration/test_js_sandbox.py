...
NO_DEFAULT_EXPORT_HANDLER = (
    'export const x = 1;'
)

SYNTAX_ERROR_HANDLER = (
    'export default { async fetch(req) { return @@@; } };'
)
...
def _make_sandbox_config(source_dir: str, worker_threads: int = 2) -> dict:
...
def _curl_get(port: int, path: str = "/", headers: dict | None = None,
...
def _curl_request(method: str, port: int, path: str = "/",
    ...
    """
    url = f"http://127.0.0.1:{port}{path}"
    cmd = ["curl", "-s", "-D", "-", "-X", method, "--max-time", str(timeout), url]
    if headers:
        for k, v in headers.items():
            cmd.extend(["-H", f"{k}: {v}"])
    if body is not None:
...
def _curl_request_binary(method: str, port: int, path: str = "/",
                          headers: dict | None = None, body: bytes | None = None,
                          timeout: int = 10) -> tuple[int, dict[str, str], bytes]:
...
def _parse_curl_output_bytes(raw: bytes) -> tuple[int, dict[str, str], bytes]:
...
class TestSandboxMissingId:
    ...
    def test_missing_sandbox_id_returns_400(self, proxy_with_config, temp_dir):
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(proxy.port)
            assert status == 400
...
class TestSandboxNotFound:
    ...
    def test_nonexistent_sandbox_returns_404(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)

        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "nonexistent"}
            )
            assert status == 404
            assert "sandbox not found" in body.lower()

    def test_empty_sandbox_id_returns_error(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)

        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port, headers={"sandbox-id": ""}
            )
...
class TestSandboxSimpleResponse:
    ...
    def test_simple_handler_returns_200(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "hello"}
            )
            assert status == 200
            assert body == "hello from sandbox"

    def test_handler_returns_custom_status(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(
            source_dir, "created",
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "created"}
            )
            assert status == 201
            assert body == "created"

    def test_handler_returns_500(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(
            source_dir, "server_err",
            'export default { async fetch(req) {'
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "server_err"}
            )
            assert status == 500
            assert body == "boom"
...
class TestSandboxHttpMethods:
    ...
    def test_get_request(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_request(
                "GET", proxy.port, headers={"sandbox-id": "echo_method"}
            )
            assert status == 200
            assert body == "GET"

    def test_post_request(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "echo_method", ECHO_METHOD_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_request(
                "POST", proxy.port, headers={"sandbox-id": "echo_method"}
            )
            assert status == 200
            assert body == "POST"

    def test_put_request(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "echo_method", ECHO_METHOD_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_request(
                "PUT", proxy.port, headers={"sandbox-id": "echo_method"}
            )
            assert status == 200
            assert body == "PUT"

    def test_delete_request(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "echo_method", ECHO_METHOD_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_request(
                "DELETE", proxy.port, headers={"sandbox-id": "echo_method"}
            )
            assert status == 200
            assert body == "DELETE"

    def test_patch_request(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "echo_method", ECHO_METHOD_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_request(
                "PATCH", proxy.port, headers={"sandbox-id": "echo_method"}
            )
            assert status == 200
            assert body == "PATCH"
...
class TestSandboxRequestBody:
    ...
    def test_post_body_echo(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
    ...
    def test_json_request_parsing(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
...
class TestSandboxRequestHeaders:
    ...
    def test_custom_header_echo(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
...
class TestSandboxRequestUrl:
    ...
    def test_url_path_echo(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
...
class TestSandboxResponseHeaders:
    ...
    def test_custom_response_headers(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "custom_hdr"}
            )
            assert status == 200
            assert headers.get("x-response") == "from-sandbox"
            assert "text/plain" in headers.get("content-type", "")

    def test_json_response_content_type(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(
            source_dir, "json_resp",
            'export default { async fetch(req) {'
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "json_resp"}
            )
            assert status == 200
...
class TestSandboxJsErrors:
    ...
    def test_handler_throws_returns_502(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "thrower"}
            )
            assert status == 502

    def test_handler_without_fetch_returns_502(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "no_fetch", NO_FETCH_HANDLER)

        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, _ = _curl_get(
                proxy.port, headers={"sandbox-id": "no_fetch"}
            )
            assert status == 502

    def test_handler_without_default_export_returns_502(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, _ = _curl_get(
                proxy.port, headers={"sandbox-id": "no_export"}
            )
            assert status == 502

    def test_syntax_error_returns_502(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, _ = _curl_get(
                proxy.port, headers={"sandbox-id": "bad_syntax"}
            )
            assert status == 502

    def test_handler_returns_non_response_returns_502(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, _ = _curl_get(
                proxy.port, headers={"sandbox-id": "bad_return"}
            )
            assert status == 502
...
class TestSandboxResponseBody:
    ...
    def test_empty_body(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port, headers={"sandbox-id": "empty_body"}
            )
            assert status == 200
            assert body == ""

    def test_no_content_204(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port, headers={"sandbox-id": "no_content"}
            )
            assert status == 204

    def test_unicode_body(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_get(
                proxy.port, headers={"sandbox-id": "unicode"}
            )
            assert status == 200
            assert body == "你好世界"

    def test_large_body(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port, headers={"sandbox-id": "large_body"}
            )
            assert status == 200
            assert len(body) == 100000
            assert body == "x" * 100000

    def test_binary_body(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "binary_body", BINARY_BODY_HANDLER)

        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, headers, body = _curl_request_binary(
                "GET",
                proxy.port,
...
class TestSandboxCustomLimits:
    ...
    def test_custom_mem_header_accepted(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
    ...
    def test_custom_cpu_header_accepted(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
    ...
    def test_invalid_mem_uses_default(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port,
                headers={"sandbox-id": "hello", "sandbox-mem": "not-a-number"},
            )
            assert status == 200
            assert body == "hello from sandbox"

    def test_invalid_cpu_uses_default(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status, _, body = _curl_get(
                proxy.port,
                headers={"sandbox-id": "hello", "sandbox-cpu": "garbage"},
            )
            assert status == 200
            assert body == "hello from sandbox"
...
class TestSandboxMultipleHandlers:
    ...
    def test_different_handlers(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(
            source_dir, "handler_a",
            'export default { async fetch(req) {'
            '  return new Response("handler-a", { status: 200 });'
            '}};'
        )
        _write_js(
            source_dir, "handler_b",
            'export default { async fetch(req) {'
        ...
        with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
            status_a, _, body_a = _curl_get(
                proxy.port, headers={"sandbox-id": "handler_a"}
            )
            assert status_a == 200
            assert body_a == "handler-a"

            status_b, _, body_b = _curl_get(
                proxy.port, headers={"sandbox-id": "handler_b"}
            )
            assert status_b == 200
...
class TestSandboxSequentialRequests:
    ...
    def test_sequential_requests(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)
...
class TestSandboxConcurrentRequests:
    ...
    def test_concurrent_requests(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)
        _write_js(source_dir, "hello", SIMPLE_HANDLER)

        with proxy_with_config(_make_sandbox_config(source_dir, worker_threads=4)) as proxy:
            results: list[Optional[tuple[int, str]]] = [None] * 10
            errors = []
...
class TestSandboxOutboundFetch:
    ...
    def test_handler_makes_outbound_fetch(self, proxy_with_config, temp_dir):
        source_dir = os.path.join(temp_dir, "scripts")
        os.makedirs(source_dir)

        # Start a local HTTP target server
        target_port = get_unique_port()
        thread, target_sock = create_target_server(
            "127.0.0.1", target_port, http_echo_handler
        )
        assert wait_for_proxy("127.0.0.1", target_port, timeout=2.0), \
        ...
        try:
            ...
            with proxy_with_config(_make_sandbox_config(source_dir)) as proxy:
                status, _, body = _curl_get(
                    proxy.port, headers={"sandbox-id": "fetch_upstream"}
                )
                assert status == 200
                assert "OK" in body
        finally:
