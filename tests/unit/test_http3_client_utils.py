...
@asynccontextmanager
async def failing_h3_connect(*args: object, **kwargs: object) -> AsyncGenerator[None]:
...
class TestH3CustomAuthority:
    ...
    async def test_custom_authority_sent_correctly(self) -> None:
        ...
        sig = inspect.signature(http3_client.perform_h3_request_with_custom_authority)
    ...
    @pytest.mark.asyncio
    async def test_custom_authority_function_returns_h3response(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @pytest.mark.asyncio
    async def test_custom_authority_with_additional_headers(self, monkeypatch: pytest.MonkeyPatch) -> None:
